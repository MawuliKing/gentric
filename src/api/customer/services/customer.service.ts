...
export class CustomerService {
    ...
    async create(createCustomerDto: CreateCustomerDto): Promise<StructuredResponse> {
        ...
        const { password, emailVerificationToken: _, ...customerResponse } = savedCustomer;
    ...
    async update(id: string, updateCustomerDto: UpdateCustomerDto): Promise<StructuredResponse> {
        const customer = await this.identityRepository.findOne({
            where: { id, type: ACCOUNT_TYPE.CUSTOMER },
        });
        ...
        // Remove sensitive data from response
        const { password, emailVerificationToken, ...customerResponse } = updatedCustomer;
